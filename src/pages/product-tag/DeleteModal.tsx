...
import { deleteProductTagAsyncThunk, getProductTagState } from '@/features/redux/slices/product-tag';
...
const DeleteModal: React.FC<IDeleteModal> = ({ id }) => {
  ...
  const { deleteLoading, deleteError } = useAppSelector(getProductTagState);
  ...
  const handleDelete = (e: React.BaseSyntheticEvent) => {
    e.preventDefault();
    dispatch(deleteProductTagAsyncThunk(id));
