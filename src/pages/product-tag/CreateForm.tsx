...
import { createProductTagAsyncThunk, getProductTagState, setProductId, setTagId } from '@/features/redux/slices/product-tag';
...
const CreateForm: React.FC<ICreateForm> = ({ handleCloseModalCreate }) => {
  ...
  const { dataInput, postLoading, postError } = useAppSelector(getProductTagState);
  const dispatch = useAppDispatch();

  const onSubmit: SubmitHandler<Inputs> = async (data) => {
    console.log('🚀 ~ file: CreateForm.tsx ~ line 33 ~ constonSubmit:SubmitHandler<Inputs>= ~ data', data);
    await dispatch(createProductTagAsyncThunk(data));
          ...
          <React.Fragment>
            <Box component="form" onSubmit={handleSubmit(onSubmit)}>
              <Grid container spacing={3}>
                ...
                <Grid item xs={12} sm={12}>
                  <TextField
                    {...register('tagId', { required: 'Required' })}
                    type="number"
                    error={errors.tagId ? true : false}
                    id="outlined-error-helper-text"
                    label="tagId"
                    placeholder="Enter tagId"
                    helperText={errors.tagId ? String(errors.tagId.message) : ''}
                    fullWidth
                    value={dataInput.tagId}
                    onChange={(e: React.BaseSyntheticEvent) => dispatch(setTagId(e.target.value))}
                  />
                </Grid>
                <Grid item xs={12}>
                  {postLoading === 'failed' ? (
