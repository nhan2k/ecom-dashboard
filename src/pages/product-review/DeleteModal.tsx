...
import { deleteProductReviewAsyncThunk, getProductReviewState } from '@/features/redux/slices/product-review';
...
const DeleteModal: React.FC<IDeleteModal> = ({ id }) => {
  ...
  const { deleteLoading, deleteError } = useAppSelector(getProductReviewState);
  ...
  const handleDelete = (e: React.BaseSyntheticEvent) => {
    e.preventDefault();
    dispatch(deleteProductReviewAsyncThunk(id));
