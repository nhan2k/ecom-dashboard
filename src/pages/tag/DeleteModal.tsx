...
import { deleteTagAsyncThunk, getTagState } from '@/features/redux/slices/tag';
...
const DeleteModal: React.FC<IDeleteModal> = ({ id }) => {
  ...
  const { deleteLoading, deleteError } = useAppSelector(getTagState);
  ...
  const handleDelete = (e: React.BaseSyntheticEvent) => {
    e.preventDefault();
    dispatch(deleteTagAsyncThunk(id));
