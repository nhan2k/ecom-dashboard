...
import { deleteProductMetaAsyncThunk, getProductMetaState } from '@/features/redux/slices/product-meta';
...
const DeleteModal: React.FC<IDeleteModal> = ({ id }) => {
  ...
  const { deleteLoading, deleteError } = useAppSelector(getProductMetaState);
  ...
  const handleDelete = (e: React.BaseSyntheticEvent) => {
    e.preventDefault();
    dispatch(deleteProductMetaAsyncThunk(id));
