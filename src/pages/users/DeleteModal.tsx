...
import { deleteUserAsyncThunk, getUserState } from '@/features/redux/slices/user';
...
const DeleteModal: React.FC<IDeleteModal> = ({ id }) => {
  ...
  const { deleteLoading, deleteError } = useAppSelector(getUserState);
  ...
  const handleDelete = (e: React.BaseSyntheticEvent) => {
    e.preventDefault();
    dispatch(deleteUserAsyncThunk(id));
