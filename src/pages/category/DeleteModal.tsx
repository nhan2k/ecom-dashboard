...
import { deleteCategoryAsyncThunk, getCategoryState } from '@/features/redux/slices/category';
...
const DeleteModal: React.FC<IDeleteModal> = ({ id }) => {
  ...
  const { deleteLoading, deleteError } = useAppSelector(getCategoryState);
  ...
  const handleDelete = (e: React.BaseSyntheticEvent) => {
    e.preventDefault();
    dispatch(deleteCategoryAsyncThunk(id));
