...
import { deleteProductAsyncThunk, getProductState } from '@/features/redux/slices/product';
...
const DeleteModal: React.FC<IDeleteModal> = ({ id }) => {
  ...
  const { deleteLoading, deleteError } = useAppSelector(getProductState);
  ...
  const handleDelete = (e: React.BaseSyntheticEvent) => {
    e.preventDefault();
    dispatch(deleteProductAsyncThunk(id));
