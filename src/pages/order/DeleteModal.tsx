...
import { deleteOrderAsyncThunk, getOrderState } from '@/features/redux/slices/order';
...
const DeleteModal: React.FC<IDeleteModal> = ({ id }) => {
  ...
  const { deleteLoading, deleteError } = useAppSelector(getOrderState);
  ...
  const handleDelete = (e: React.BaseSyntheticEvent) => {
    e.preventDefault();
    dispatch(deleteOrderAsyncThunk(id));
